...
use Query_Forge\QF_Single_Template;
...
class Smart_Loop_Grid extends Widget_Base {
	...
	/**
	 * Sanitized widget id for qfr on card links (matches single-template transient key).
	 *
	 * @var string
	 */
	...
	protected function register_controls() {
		...
		if ( function_exists( '\Query_Forge\qf_register_elementor_single_template_controls' ) ) {
			\Query_Forge\qf_register_elementor_single_template_controls( $this );
		}
	...
	protected function render() {
		...
		QF_Single_Template::store_from_elementor( $widget_id, $settings );
		if ( ! empty( $settings['single_template_enabled'] ) && 'yes' === $settings['single_template_enabled'] ) {
			$this->qf_single_qfr_value = sanitize_key( $widget_id );
		}
		...
		$ctx_hash   = QF_Query_Parser::stable_hash_for_cache( $settings );
	...
	/**
	 * Post permalink with optional qfr for single-template views.
	 *
	 * @return string
	 */
	private function get_single_aware_permalink() {
		$url = get_permalink();
