...
class QF_Block {
	...
	/**
	 * Sanitized qfInstanceId suffix for single-template permalinks (matches transient key).
	 *
	 * @var string
	 */
	private $qf_single_qfr_value = '';
	...
	public function render_block( array $attributes, string $content, $block = null ): string {
		try {
			return $this->render_block_inner( $attributes, $content, $block );
		} catch ( \Throwable $e ) {
			if ( defined( 'WP_DEBUG' ) && WP_DEBUG && defined( 'WP_DEBUG_LOG' ) && WP_DEBUG_LOG ) {
				// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Opt-in debug logging only.
				error_log(
					sprintf(
						'Query Forge render_block: %s in %s:%d',
						$e->getMessage(),
						$e->getFile(),
						$e->getLine()
					)
			...
			$fallback = '<div class="qf-query-forge-root"><div class="qf-placeholder"><p>'
				. esc_html__( 'Preview could not be loaded. Check debug.log with WP_DEBUG_LOG, or try disabling other plugins to find a conflict.', 'query-forge' )
				. '</p></div></div>';
			return wp_check_invalid_utf8( $fallback, true );
		}
	}

	/**
	 * Block HTML output (throws may propagate to render_block try/catch).
	 *
	 * @param array    $attributes Block attributes.
	 * @param string   $content    Inner blocks (unused).
	 ...
	 * @return string
	 */
	private function render_block_inner( array $attributes, string $content, $block = null ): string {
		...
		$logic_raw = QF_Query_Parser::coerce_json_string( $attributes['logicJson'] ?? '' );
		$post_id   = get_the_ID();
		...
		$qf_instance_raw = isset( $attributes['qfInstanceId'] ) ? (string) $attributes['qfInstanceId'] : '';
		QF_Single_Template::store_from_block( $qf_instance_raw, $attributes );
		$this->qf_single_qfr_value = '';
		if ( ! empty( $attributes['singleTemplateEnabled'] ) && '' !== $qf_instance_raw ) {
			$this->qf_single_qfr_value = sanitize_key( $qf_instance_raw );
		}
		...
		$ctx_hash    = QF_Query_Parser::stable_hash_for_cache( $attributes );
		...
		echo '</div>';
		$html = ob_get_clean();
		return wp_check_invalid_utf8( is_string( $html ) ? $html : '', true );
	}
	...
	private function resolve_font_family_preset_slug( string $value ): string {
		...
		$ok = @preg_match( '#^[A-Za-z0-9À-ÖØ-öø-ÿ\s\',\-\."\/_]+$#u', $value );
		if ( $ok ) {
			return $value;
		}
	...
	private function maybe_enqueue_google_fonts( array $attributes ): void {
		// Avoid enqueue side effects during block REST preview (editor SSR does not load qf-widget.js).
	...
	private function attributes_to_settings( array $attributes ): array {
		return [
			'qf_logic_json'            => QF_Query_Parser::coerce_json_string( $attributes['logicJson'] ?? '' ),
			'qf_graph_state'           => QF_Query_Parser::coerce_json_string( $attributes['graphState'] ?? '' ),
			'display_type'             => 'canned',
	...
	private function render_main( array $attributes, array $settings, $paged = 1, $ppp = 10, $instance_id = '' ): void {
		...
		// Editor block SSR (REST) does not enqueue qf-widget.js — omit heavy logic_json from data attributes
		// to keep the REST JSON envelope small (avoids proxies/WAF limits and encode edge cases).
		$logic_json_for_dom = '';
		if ( ! ( defined( 'REST_REQUEST' ) && REST_REQUEST ) ) {
			$logic_json_for_dom = QF_Query_Parser::coerce_json_string( $attributes['logicJson'] ?? '' );
		}

		$data_settings = wp_json_encode(
			[
				'logic_json'      => $logic_json_for_dom,
				'widget_settings' => [
					'display_type'           => 'canned',
					'card_style'             => $attributes['cardStyle'] ?? 'vertical',
					'show_title'             => $attributes['showTitle'] ?? 'yes',
			...
			JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_UNESCAPED_SLASHES | ( defined( 'JSON_INVALID_UTF8_SUBSTITUTE' ) ? JSON_INVALID_UTF8_SUBSTITUTE : 0 )
		);
		if ( ! is_string( $data_settings ) ) {
			$data_settings = '{}';
		}
	...
	public function render_card( $settings, $style ) {
		...
		$single_attr = '';
		if ( '' !== $this->qf_single_qfr_value ) {
			$single_attr = ' data-qf-single-instance="' . esc_attr( $this->qf_single_qfr_value ) . '"';
		}
		?>
		<div class="qf-card qf-card-<?php echo esc_attr( $style ); ?>"<?php echo $single_attr; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Built with esc_attr. ?>>
			<?php
			switch ( $style ) {
				case 'horizontal':
	...
	private function render_title( $link_target, array $settings = [] ) {
		...
		<h3 class="qf-card-title">
			<a href="<?php echo esc_url( $this->get_single_aware_permalink() ); ?>" target="<?php echo esc_attr( $link_target ); ?>" style="<?php echo esc_attr( $title_style ); ?>">
	...
	private function render_read_more_button( $link_target ) {
		...
		<div class="qf-card-button-wrapper">
			<a href="<?php echo esc_url( $this->get_single_aware_permalink() ); ?>" target="<?php echo esc_attr( $link_target ); ?>" class="qf-card-button">
	...
	private function render_featured_image( $settings, $link_target ) {
		...
		if ( $image_id ) {
			...
			?>
			<a href="<?php echo esc_url( $this->get_single_aware_permalink() ); ?>" target="<?php echo esc_attr( $link_target ); ?>">
	...
	/**
	 * Permalink for card links; adds qfr when single template is active for this block.
	 *
	 * @return string
	 */
	private function get_single_aware_permalink(): string {
		$url = get_permalink();
		if ( '' === $this->qf_single_qfr_value || ! is_string( $url ) ) {
			return $url;
		}
		return add_query_arg( QF_Single_Template::QUERY_VAR, $this->qf_single_qfr_value, $url );
	}
