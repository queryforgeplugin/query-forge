...
final class Plugin {
	...
	private function includes() {
		...
		require_once QUERY_FORGE_PATH . 'includes/class-qf-single-template.php';
		require_once QUERY_FORGE_PATH . 'includes/class-qf-admin-settings.php';
	}
	...
	private function hooks() {
		add_action( 'plugins_loaded', [ $this, 'maybe_mark_legacy_filter_upgrade_pending' ], 3 );
		// Load admin hub early so admin_menu callbacks register before admin_menu runs.
		add_action( 'plugins_loaded', [ $this, 'init_admin' ], 5 );
		...
		add_action( 'wp_ajax_query_forge_export_queries', [ $this, 'ajax_export_queries' ] );
		add_action( 'wp_ajax_query_forge_import_queries', [ $this, 'ajax_import_queries' ] );
		...
		add_action( 'wp_ajax_query_forge_backup_builder_canvas', [ $this, 'ajax_backup_builder_canvas' ] );
		add_action( 'wp_ajax_query_forge_filter_autoupgrade_complete', [ $this, 'ajax_filter_autoupgrade_complete' ] );
		add_action( 'admin_notices', [ $this, 'render_activation_admin_notice' ] );
		add_action( 'admin_notices', [ $this, 'maybe_render_filter_upgrade_success_notice' ], 20 );
		...
		add_filter( 'query_vars', [ $this, 'register_qfr_query_var' ] );
		add_filter( 'template_include', [ $this, 'maybe_load_single_template' ], 99 );
		add_filter( 'get_canonical_url', [ $this, 'strip_qfr_from_canonical' ], 10, 2 );
	}

	/**
	 * Load admin settings hub (top-level menu).
	 *
	 * Must run on plugins_loaded (not admin_init) so QF_Admin_Settings can register
	 * admin_menu before WordPress builds the admin menu.
	 */
	...
	public function enqueue_editor_scripts() {
		...
		wp_localize_script(
			...
			[
				...
				'legacyFilterAutoupgradePending' => (bool) get_option( 'qf_legacy_filter_autoupgrade_pending', false ),
				'legacyFilterSunsetNote'        => __( 'Legacy filter layouts are supported for two more plugin releases. After that, graphs will upgrade automatically when you open the builder.', 'query-forge' ),
			]
		);
	...
	/**
	 * Sites that existed before query_forge_db_version was stored may have legacy filter node payloads.
	 */
	public function maybe_mark_legacy_filter_upgrade_pending(): void {
		$stored = get_option( 'query_forge_db_version', false );
		if ( false === $stored ) {
			update_option( 'qf_legacy_filter_autoupgrade_pending', true, false );
		}
		if ( $stored !== QUERY_FORGE_VERSION ) {
			update_option( 'query_forge_db_version', QUERY_FORGE_VERSION, false );
		}
	}

	/**
	 * AJAX: save current builder graph as a named snapshot (editors with edit_posts).
	 *
	 * @since 1.3.7
	 */
	public function ajax_backup_builder_canvas() {
		check_ajax_referer( 'query_forge_nonce', 'nonce' );
		if ( ! current_user_can( 'edit_posts' ) ) {
			wp_send_json_error( [ 'message' => __( 'Permission denied.', 'query-forge' ) ] );
			return;
		...
		// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- JSON validated and sanitized recursively below.
		...
		if ( false !== $saved ) {
			...
			wp_send_json_success(
				[
					'message' => __( 'Backup saved to Saved Queries.', 'query-forge' ),
					'id'      => $query_id,
				]
			);
		} else {
			wp_send_json_error( [ 'message' => __( 'Failed to save backup.', 'query-forge' ) ] );
		}
	}

	/**
	 * AJAX: filter nodes were rewritten to canonical multi-condition data in the editor.
	 */
	public function ajax_filter_autoupgrade_complete() {
		check_ajax_referer( 'query_forge_nonce', 'nonce' );
		if ( ! current_user_can( 'edit_posts' ) ) {
			wp_send_json_error( [ 'message' => __( 'Permission denied.', 'query-forge' ) ] );
			return;
		}
		delete_option( 'qf_legacy_filter_autoupgrade_pending' );
		$uid = get_current_user_id();
		if ( $uid ) {
			set_transient( 'qf_filter_upgrade_notice_' . $uid, 1, DAY_IN_SECONDS );
		}
		wp_send_json_success();
	}

	/**
	 * One-shot admin notice after the editor reports an autoupgrade.
	 */
	public function maybe_render_filter_upgrade_success_notice(): void {
		...
		$key = 'qf_filter_upgrade_notice_' . $uid;
		if ( ! get_transient( $key ) ) {
			return;
		}
		delete_transient( $key );
		echo '<div class="notice notice-success is-dismissible"><p>';
		echo esc_html__( 'Query Forge upgraded filters you migrated in the editor to the new multi-condition format. Save each affected query to persist the change.', 'query-forge' );
		echo '</p></div>';
	}
	...
	/**
	 * Build sanitized query option array (same shape as ajax_save_query).
	 *
	 * @param array $raw_query_data Raw keys: name, optional date, graphState, logicJson strings.
	 * @return array<string, string>|null
	 */
	private function build_query_data_from_raw( array $raw_query_data ): ?array {
		if ( empty( $raw_query_data['name'] ) || ! is_string( $raw_query_data['name'] ) ) {
			return null;
		}

		$sanitized_name = sanitize_text_field( $raw_query_data['name'] );
		$sanitized_date = isset( $raw_query_data['date'] ) && is_string( $raw_query_data['date'] )
			? sanitize_text_field( $raw_query_data['date'] )
			: current_time( 'mysql' );
	...
	/**
	 * AJAX: export selected saved queries as JSON (Free: 5 lifetime exports).
	 */
	public function ajax_export_queries() {
		check_ajax_referer( 'query_forge_nonce', 'nonce' );
		...
		$raw_ids = isset( $_POST['query_ids'] ) ? wp_unslash( $_POST['query_ids'] ) : '[]';
		$ids     = is_string( $raw_ids ) ? json_decode( $raw_ids, true ) : null;
		if ( ! is_array( $ids ) ) {
			wp_send_json_error( [ 'message' => __( 'Invalid request.', 'query-forge' ) ] );
			return;
		}
		...
		$valid_ids = [];
		foreach ( $ids as $id ) {
			if ( ! is_string( $id ) && ! is_scalar( $id ) ) {
				continue;
			}
			$id = sanitize_text_field( (string) $id );
			if ( isset( $query_list[ $id ] ) ) {
				$valid_ids[] = $id;
			}
		}
		...
		if ( empty( $valid_ids ) ) {
			wp_send_json_error( [ 'message' => __( 'No valid queries selected.', 'query-forge' ) ] );
			return;
		}

		$export_count = (int) get_option( 'query_forge_export_count', 0 );
		$export_limit = 5;
		$remaining    = max( 0, $export_limit - $export_count );

		if ( $remaining <= 0 ) {
			wp_send_json_error(
				[
					'message' => __( 'You have used your 5-query export limit. Upgrade to Pro for unlimited exports.', 'query-forge' ),
				]
			);
			return;
		}

		$valid_ids = array_slice( $valid_ids, 0, $remaining );
		...
		$queries_payload = [];
		foreach ( $valid_ids as $query_id ) {
			$full = get_option( $query_id, null );
			if ( ! is_array( $full ) ) {
				continue;
			}
			$queries_payload[] = array_merge( [ 'id' => $query_id ], $full );
		}

		if ( empty( $queries_payload ) ) {
			wp_send_json_error( [ 'message' => __( 'Could not read selected queries.', 'query-forge' ) ] );
			return;
		}

		$exported_count = count( $queries_payload );
		$new_total      = $export_count + $exported_count;
		update_option( 'query_forge_export_count', $new_total, false );
		...
		wp_send_json_success(
			[
				...
				'remaining'      => max( 0, $export_limit - $new_total ),
			]
		);
	}

	/**
	 * AJAX: import queries from uploaded JSON.
	 */
	public function ajax_import_queries() {
		check_ajax_referer( 'query_forge_nonce', 'nonce' );
		...
		if ( empty( $_FILES['qf_import_file'] ) || ! is_array( $_FILES['qf_import_file'] ) ) {
			wp_send_json_error( [ 'message' => __( 'No file uploaded.', 'query-forge' ) ] );
			return;
		}

		$file = $_FILES['qf_import_file'];
		if ( ! empty( $file['error'] ) || ! isset( $file['tmp_name'] ) || ! is_uploaded_file( $file['tmp_name'] ) ) {
			wp_send_json_error( [ 'message' => __( 'Upload failed.', 'query-forge' ) ] );
			return;
		}

		$name = isset( $file['name'] ) ? (string) $file['name'] : '';
		if ( ! preg_match( '/\.json$/i', $name ) ) {
			wp_send_json_error( [ 'message' => __( 'File must be a .json file.', 'query-forge' ) ] );
			return;
		}

		if ( isset( $file['size'] ) && (int) $file['size'] > 1048576 ) {
			wp_send_json_error( [ 'message' => __( 'File is too large. Maximum size is 1MB.', 'query-forge' ) ] );
			return;
		}

		// phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents -- Validated upload path.
		$contents = file_get_contents( $file['tmp_name'] );
		if ( false === $contents || '' === $contents ) {
			wp_send_json_error( [ 'message' => __( 'Could not read file.', 'query-forge' ) ] );
			return;
		}

		$data = json_decode( $contents, true );
		if ( json_last_error() !== JSON_ERROR_NONE || ! is_array( $data ) || empty( $data['queries'] ) || ! is_array( $data['queries'] ) ) {
			wp_send_json_error( [ 'message' => __( 'Invalid import file.', 'query-forge' ) ] );
			return;
		}
		...
		$existing_lower = [];
		foreach ( $query_list as $meta ) {
			if ( is_array( $meta ) && ! empty( $meta['name'] ) ) {
				$existing_lower[ strtolower( (string) $meta['name'] ) ] = true;
			}
		}
		...
		foreach ( $data['queries'] as $index => $item ) {
			if ( ! is_array( $item ) ) {
				$errors[] = [
					'index'  => (int) $index,
					'reason' => __( 'Entry is not a valid object.', 'query-forge' ),
				];
				continue;
			}
			...
			if ( empty( $item['name'] ) || ! is_string( $item['name'] ) ) {
				$errors[] = [
					'index'  => (int) $index,
					'reason' => __( 'Missing or invalid query name.', 'query-forge' ),
				];
				continue;
			}

			$logic_json = '';
			if ( isset( $item['logicJson'] ) && is_string( $item['logicJson'] ) ) {
				$logic_json = $item['logicJson'];
			}

			if ( ! isset( $item['graphState'] ) || ! is_string( $item['graphState'] ) || '' === trim( $item['graphState'] ) ) {
				$errors[] = [
					'index'  => (int) $index,
					'reason' => __( 'graphState is required and must be a non-empty string.', 'query-forge' ),
				];
				continue;
			}

			$graph_decoded = json_decode( $item['graphState'], true );
			if ( json_last_error() !== JSON_ERROR_NONE || ! is_array( $graph_decoded ) ) {
				$errors[] = [
					'index'  => (int) $index,
					'reason' => __( 'graphState must be valid JSON encoding an array.', 'query-forge' ),
				];
				continue;
			}

			$desired = sanitize_text_field( $item['name'] );
			$base    = preg_replace( '/\s+\d+$/', '', $desired );
			$base    = trim( $base );
			if ( '' === $base ) {
				$base = __( 'Imported query', 'query-forge' );
			}

			$candidate = $base;
			$n         = 2;
			while ( isset( $existing_lower[ strtolower( $candidate ) ] ) ) {
				$candidate = $base . ' ' . $n;
				$n++;
			}

			$existing_lower[ strtolower( $candidate ) ] = true;
			...
			$query_data = $this->build_query_data_from_raw( $raw_row );
			if ( null === $query_data ) {
				$errors[] = [
					'index'  => (int) $index,
					'reason' => __( 'Could not sanitize query data.', 'query-forge' ),
				];
				unset( $existing_lower[ strtolower( $candidate ) ] );
				continue;
			}

			$query_id = 'query_forge_query_' . md5( $candidate . time() . uniqid( '', true ) );
			$saved    = update_option( $query_id, $query_data, false );
			if ( false === $saved ) {
				$errors[] = [
					'index'  => (int) $index,
					'reason' => __( 'Failed to save query.', 'query-forge' ),
				];
				unset( $existing_lower[ strtolower( $candidate ) ] );
				continue;
			}
	...
	/**
	 * Register public query var for single-template instance id (?qfr=).
	 *
	 * @param string[] $vars Query vars.
	 * @return string[]
	 */
	public function register_qfr_query_var( $vars ) {
		$vars[] = \Query_Forge\QF_Single_Template::QUERY_VAR;
		return $vars;
	}

	/**
	 * Load Query Forge single layout when qfr matches a stored instance.
	 *
	...
	public function maybe_load_single_template( $template ) {
		...
		$qfr = get_query_var( \Query_Forge\QF_Single_Template::QUERY_VAR );
		if ( ! is_string( $qfr ) || '' === $qfr ) {
			// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Public navigation parameter.
			if ( isset( $_GET[ \Query_Forge\QF_Single_Template::QUERY_VAR ] ) ) {
				$qfr = sanitize_text_field( wp_unslash( (string) $_GET[ \Query_Forge\QF_Single_Template::QUERY_VAR ] ) );
			}
		}
		if ( '' === $qfr ) {
			return $template;
		...
		$suffix   = sanitize_key( $qfr );
		$settings = get_transient( 'qf_single_instance_' . $suffix );
		if ( ! is_array( $settings ) || empty( $settings['style'] ) ) {
			return $template;
		}

		$style = sanitize_key( (string) $settings['style'] );
		if ( ! in_array( $style, \Query_Forge\QF_Single_Template::allowed_styles(), true ) ) {
			$style = 'vertical';
		}

		$file = QUERY_FORGE_PATH . 'templates/single/single-' . $style . '.php';
		if ( ! is_readable( $file ) ) {
			return $template;
		}
		...
		$qf_post     = $queried;
		$qf_settings = $settings;

		return $file;
	}

	/**
	 * Remove qfr from canonical URLs (SEO).
	 *
	 * @param string   $canonical_url Canonical URL.
	 * @param \WP_Post $post          Post.
	 * @return string
	 */
	public function strip_qfr_from_canonical( $canonical_url, $post ) {
		unset( $post );
		if ( ! is_string( $canonical_url ) || '' === $canonical_url ) {
			return $canonical_url;
		}
		return remove_query_arg( \Query_Forge\QF_Single_Template::QUERY_VAR, $canonical_url );
	}
