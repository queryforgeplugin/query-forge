...
class QF_Query_Parser {
	...
	/**
	 * Block attributes and stored values should be JSON strings; coerce arrays/objects (or broken casts) safely.
	 *
	 * @param mixed $data Raw logic/graph JSON or decoded structure.
	 * @return string JSON text for json_decode / persistence.
	 */
	public static function coerce_json_string( $data ): string {
		if ( is_string( $data ) ) {
			return $data;
		}
	...
	/**
	 * MD5 of JSON-serialized data for cache context keys (never pass wp_json_encode(false) to md5 on PHP 8+).
	 *
	 * @param mixed $data Array/object for block attributes or widget settings, or string (hashed as-is).
	 * @return string 32-char hex.
	 */
	public static function stable_hash_for_cache( $data ): string {
		if ( is_string( $data ) ) {
			return md5( $data );
		}
		if ( is_array( $data ) || is_object( $data ) ) {
			...
			return md5( is_string( $json ) ? $json : '' );
		}
		return md5( '' );
	}
	 ...
	 *
	 * @since 1.3.3
	 * @param mixed $json_data Logic JSON string or decoded structure.
	 * @return int>=1
	 */
	public static function resolve_posts_per_page_for_query( $json_data ) {
		$data = json_decode( self::coerce_json_string( $json_data ), true );
	 ...
	 *
	 * @since 1.0.0
	 * @param mixed    $json_data       JSON schema string or decoded array/object (coerced).
	...
	public static function get_query( $json_data, $paged = null, $posts_per_page = null, $extra_args = null ) {
		$json_data = self::coerce_json_string( $json_data );

		// Detect if we're in Elementor preview/editor mode or on frontend (null-safe: avoids fatals during REST / block SSR).
		$is_preview = false;
		...
		try {
			...
			// SECURITY FIX: Ensure filters are removed even on error (Throwable catches TypeError etc., not only Exception).
			self::remove_join_filters();
			self::remove_where_filters();
	...
	private static function query_contains_post_status_filter( $query_node ) {
		...
		if ( ! empty( $query_node['filter'] ) && is_array( $query_node['filter'] ) ) {
			...
			if ( ! empty( $f['clauses'] ) && is_array( $f['clauses'] ) ) {
				foreach ( $f['clauses'] as $sub ) {
					if ( is_array( $sub ) && isset( $sub['field'] ) && 'post_status' === $sub['field'] ) {
						return true;
					}
				}
			}
		...
		if ( ! empty( $query_node['paths'] ) && is_array( $query_node['paths'] ) ) {
			foreach ( $query_node['paths'] as $path ) {
				if ( self::query_contains_post_status_filter( $path ) ) {
					return true;
				}
			}
		}
	...
	/**
	 * True when a clause must be skipped because operands do not satisfy the operator (editor parity).
	 *
	 * @param array $clause Filter clause.
	 * @return bool
	 */
	private static function filter_clause_drop_missing_operand( array $clause ) {
		$operator = isset( $clause['operator'] ) ? strtoupper( sanitize_text_field( $clause['operator'] ) ) : '=';
		if ( in_array( $operator, [ 'EXISTS', 'NOT EXISTS' ], true ) ) {
			return false;
		}
		if ( 'BETWEEN' === $operator ) {
			$v = isset( $clause['value'] ) ? $clause['value'] : null;
			if ( ! is_array( $v ) || ! array_key_exists( 'from', $v ) || ! array_key_exists( 'to', $v ) ) {
				return true;
			}
			return '' === trim( (string) $v['from'] ) || '' === trim( (string) $v['to'] );
		}
		$requires = [ '=', '!=', 'LIKE', 'NOT LIKE', 'IN', 'NOT IN' ];
		if ( ! in_array( $operator, $requires, true ) ) {
			return false;
		}
		...
		if ( is_string( $val ) && in_array( $operator, [ 'IN', 'NOT IN' ], true ) ) {
	...
	/**
	 * Build one tax_query clause array from a filter clause (field tax:{name}).
	 *
	 * @param array $filter Filter clause.
	 * @return array|null Inner tax_query entry or null.
	 */
	private static function filter_clause_to_tax_query_clause( array $filter ) {
		...
		$terms     = array_filter( array_map( 'trim', explode( ',', is_scalar( $raw_value ) ? (string) $raw_value : '' ) ) );
		if ( empty( $terms ) ) {
			return null;
		}
	...
	/**
	 * Execute grouped filter { relation, clauses } from schema transformer.
	 *
	 * @param array      $group     Group object.
	 * @param array      $base_args Base WP_Query args.
	 * @param array|null $post_in   Optional post__in restriction.
	 * @return array Post IDs.
	 */
	private static function run_grouped_filter_for_ids( array $group, $base_args, $post_in = null ) {
		$relation = isset( $group['relation'] ) ? strtoupper( sanitize_text_field( $group['relation'] ) ) : 'AND';
		$raw      = isset( $group['clauses'] ) && is_array( $group['clauses'] ) ? $group['clauses'] : [];
		$clauses  = [];
		foreach ( $raw as $c ) {
			if ( ! is_array( $c ) || empty( $c['field'] ) ) {
				continue;
			}
			if ( self::filter_clause_drop_missing_operand( $c ) ) {
				continue;
			}
			$clauses[] = $c;
		}
		...
		if ( count( $clauses ) === 1 ) {
			return self::run_filter_for_ids( $clauses[0], $base_args, $post_in );
		}
		if ( 'OR' === $relation ) {
			$branch_results = [];
			foreach ( $clauses as $c ) {
				$branch_results[] = self::run_filter_for_ids( $c, $base_args, $post_in );
			}
			return self::combine_logic_ids( 'OR', $branch_results );
		}
		return self::run_and_grouped_filter_for_ids( $clauses, $base_args, $post_in );
	}

	/**
	 * AND-merge multiple clauses into one WP_Query (standard + tax + meta each AND-combined).
	 *
	 * @param array      $clauses   Flat clause list.
	 * @param array      $base_args Base WP_Query args.
	 * @param array|null $post_in   Optional post__in restriction.
	 * @return array Post IDs.
	 */
	private static function run_and_grouped_filter_for_ids( array $clauses, $base_args, $post_in = null ) {
		$args                     = array_merge( [], $base_args );
		...
		foreach ( $clauses as $c ) {
			$field = isset( $c['field'] ) ? sanitize_text_field( $c['field'] ) : '';
			if ( self::is_standard_field( $field ) ) {
				$standard_clauses[] = $c;
			} elseif ( strpos( $field, 'tax:' ) === 0 ) {
				$tax_clauses[] = $c;
			} else {
				$mc = self::build_meta_clause( $c );
				if ( ! empty( $mc ) ) {
					$meta_built[] = $mc;
				}
			}
		...
		if ( ! empty( $standard_clauses ) ) {
			self::apply_standard_filters( $args, [ 'clauses' => $standard_clauses ] );
		}

		if ( ! empty( $tax_clauses ) ) {
			$tax_parts = [];
			foreach ( $tax_clauses as $tc ) {
				$p = self::filter_clause_to_tax_query_clause( $tc );
				if ( ! empty( $p ) ) {
					$tax_parts[] = $p;
				}
			}
	...
	private static function run_filter_for_ids( $filter, $base_args, $post_in = null ) {
		...
		if ( ! empty( $filter['clauses'] ) && is_array( $filter['clauses'] ) ) {
			return self::run_grouped_filter_for_ids( (array) $filter, $base_args, $post_in );
		}
		...
		if ( self::filter_clause_drop_missing_operand( $filter ) ) {
			return self::run_query_for_ids( $base_args, $post_in );
		}
		...
		if ( self::is_standard_field( $field ) ) {
			...
			$tax_inner = self::filter_clause_to_tax_query_clause( $filter );
			if ( empty( $tax_inner ) ) {
				return self::run_query_for_ids( $base_args, $post_in );
			}
	...
	private static function build_meta_clause( $clause ) {
		...
		$field = isset( $clause['field'] ) ? sanitize_text_field( $clause['field'] ) : '';
		if ( empty( $field ) ) {
			return null;
		}
		...
		if ( 'BETWEEN' === $operator ) {
			...
			if ( is_array( $v ) && isset( $v['from'], $v['to'] ) ) {
				$meta_clause['value'] = [
					sanitize_text_field( (string) $v['from'] ),
					sanitize_text_field( (string) $v['to'] ),
				];
			}
			return $meta_clause;
		}
		...
		if ( in_array( $operator, [ 'IN', 'NOT IN' ], true ) ) {
			$val = isset( $clause['value'] ) ? $clause['value'] : '';
			if ( is_array( $val ) ) {
				$meta_clause['value'] = array_map(
					static function ( $x ) {
						return sanitize_text_field( (string) $x );
					},
					$val
				);
			} else {
				$parts = array_filter( array_map( 'trim', explode( ',', (string) $val ) ) );
				$meta_clause['value'] = array_map( 'sanitize_text_field', $parts );
			}
			return $meta_clause;
		}
	...
	private static function apply_standard_filters( &$args, $filters ) {
		...
		// Multiple standard clauses are AND-only at the SQL level; OR across mixed standard columns is not modeled here.
		...
		foreach ( $filters['clauses'] as $clause ) {
			...
			switch ( $field ) {
				...
				case 'post_modified':
					...
					if ( $operator === '=' ) {
						$date_query['year'] = gmdate( 'Y', strtotime( (string) $value ) );
						$date_query['month'] = gmdate( 'm', strtotime( (string) $value ) );
						$date_query['day'] = gmdate( 'd', strtotime( (string) $value ) );
					} elseif ( $operator === 'BETWEEN' && is_array( $value ) && isset( $value['from'], $value['to'] ) ) {
						$date_query['after']     = sanitize_text_field( (string) $value['from'] );
						$date_query['before']    = sanitize_text_field( (string) $value['to'] );
