...
(function($) {
	...
	// Single-template: post links from cards with data-qf-single-instance must carry ?qfr=.
	$(document).on('click', '.qf-card[data-qf-single-instance] a[href]', function(e) {
		var $link = $(this);
		var href = $link.attr('href');
		if (!href || href.indexOf('mailto:') === 0 || href.indexOf('tel:') === 0) {
			return;
		}
		var $card = $link.closest('.qf-card[data-qf-single-instance]');
		if (!$card.length) {
			return;
		}
		var instanceId = $card.attr('data-qf-single-instance');
		if (!instanceId) {
			return;
		}
		e.preventDefault();
		...
		parsed.searchParams.set('qfr', instanceId);
