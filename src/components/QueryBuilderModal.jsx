...
import { isLegacyFilterNodeShape } from '../utils/normalizeFilterNode.js';
import { convertLegacyFiltersOnCanvas } from '../utils/migrateFilterCanvas.js';
...
function stripNeedsValidationFromNodes(nodeList) {
  return nodeList.map((node) => {
    if (
      node.type !== 'filter' ||
      !node.data ||
      !Object.prototype.hasOwnProperty.call(node.data, '_needsValidation')
    ) {
      return node;
    }
    const { _needsValidation, ...rest } = node.data;
    return { ...node, data: rest };
  });
}

const QueryBuilderModal = ({ initialData, onSave, onClose, backupContextLabel = '' }) => {
  ...
  /** Hide legacy migration card until modal/canvas reload (unmount or import replaces graph). */
  const [legacyMigrationOverlayDismissed, setLegacyMigrationOverlayDismissed] = useState(false);
  ...
  const showFilterMigrationBanner = React.useMemo(() => {
    const hasLegacy = nodes.some((n) => n.type === 'filter' && isLegacyFilterNodeShape(n.data));
    const needsReview = nodes.some(
      (n) => n.type === 'filter' && n.data && n.data._migrationReviewPending === true
    );
    return hasLegacy || needsReview;
  }, [nodes]);

  const showLegacyMigrationOverlay =
    showFilterMigrationBanner && !legacyMigrationOverlayDismissed;

  const pingFilterAutoupgradeAjax = useCallback(() => {
    const ajaxUrl = window.QueryForgeConfig?.ajaxUrl;
    const nonce = window.QueryForgeConfig?.nonce || '';
    if (!ajaxUrl || !nonce) {
      return;
    ...
    fetch(ajaxUrl, {
      ...
      body: new URLSearchParams({
        action: 'query_forge_filter_autoupgrade_complete',
        nonce,
      }),
    })
      .then(() => {
        if (typeof window !== 'undefined' && window.QueryForgeConfig) {
          window.QueryForgeConfig.legacyFilterAutoupgradePending = false;
        }
      })
      .catch(() => {});
  }, []);
  ...
  const handleCreateBackup = useCallback(async () => {
    ...
    const nonce = window.QueryForgeConfig?.nonce || '';
    if (!ajaxUrl || !nonce) {
      alert(__('Missing editor configuration. Cannot save backup.', 'query-forge'));
      return false;
    }
    const label = String(backupContextLabel || '').trim() || __('Page', 'query-forge');
    const stamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
    const name = `${label} backup ${stamp}`;
    const graphState = JSON.stringify({ nodes, edges });
    const { schema } = transformToSchema(nodes, edges);
    ...
    try {
      const res = await fetch(ajaxUrl, {
        ...
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: new URLSearchParams({
          action: 'query_forge_backup_builder_canvas',
          nonce,
      ...
      if (!data.success) {
        alert(data.data?.message || __('Backup failed.', 'query-forge'));
        return false;
      }
      alert(data.data?.message || __('Backup saved to Saved Queries.', 'query-forge'));
      return true;
    } catch (err) {
      alert(__('Network error while saving backup.', 'query-forge'));
      return false;
    }
  }, [nodes, edges, backupContextLabel]);

  const handleConvertLegacyFilters = useCallback(() => {
    if (
      !window.confirm(
        __(
          'Convert legacy filters to the new format now? Use Create Backup first if you need a restore point in Saved Queries.',
          'query-forge'
        )
      )
    ) {
    ...
    const { nodes: nn, edges: ne } = convertLegacyFiltersOnCanvas(nodes, edges);
    setNodes(nn);
    setEdges(ne);
    setSelectedNode((sel) => {
      if (!sel) {
      ...
      return nn.some((x) => x.id === sel.id) ? sel : null;
    });
    pingFilterAutoupgradeAjax();
  }, [nodes, edges, pingFilterAutoupgradeAjax, setNodes, setEdges]);

  const handleDeferLegacyMigration = useCallback(() => {
    alert(
      __(
        'No changes were made. This reminder will appear again the next time you open the Query Builder. Legacy filter layouts remain supported for two more plugin releases; after that they will upgrade automatically when you open the builder. Use Create Backup first so you can restore from Saved Queries.',
        'query-forge'
      )
    );
    setLegacyMigrationOverlayDismissed(true);
  }, []);
  ...
  const handleAddNode = (type, position) => {
    ...
    const defaultData = {
      ...
      filter: {
        label: 'Filter',
        relation: 'AND',
        conditions: [{ field: '', operator: '=', value: '', valueType: 'CHAR' }],
      },
  ...
  const handleSave = () => {
    ...
    const persistedNodes = stripNeedsValidationFromNodes(nodes);
    const { schema } = transformToSchema(persistedNodes, edges);
    const graphState = JSON.stringify({ nodes: persistedNodes, edges });
    const logicJson = JSON.stringify(schema);
          ...
          {showLegacyMigrationOverlay && (
            ...
            >
              <div style={{ fontWeight: 700, marginBottom: '8px', fontSize: '14px' }}>
                {__('Legacy Filter Nodes Detected', 'query-forge')}
              </div>
              <div style={{ lineHeight: 1.55, marginBottom: '14px' }}>
                {__(
                  'Steps: 1) Create Backup if you want a snapshot in Saved Queries 2) Convert Legacy Filters 3) Save Logic.',
                  'query-forge'
                )}
              </div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', alignItems: 'center' }}>
                ...
                >
                  {__('Create Backup', 'query-forge')}
                </button>
                <button
                  type="button"
                  onClick={() => handleConvertLegacyFilters()}
                  style={{
                    background: '#2d3748',
                    color: '#fff',
                    border: '1px solid #4a5568',
                ...
                >
                  {__('Convert Legacy Filters', 'query-forge')}
                </button>
                <button
                  type="button"
                  onClick={handleDeferLegacyMigration}
                  style={{
                    background: 'transparent',
                    color: '#fefcbf',
                    border: '1px solid rgba(254,252,191,0.5)',
                    borderRadius: '6px',
                    padding: '8px 14px',
                    cursor: 'pointer',
                    fontSize: '12px',
                ...
                >
                  {__('Not now', 'query-forge')}
                </button>
              </div>
              {typeof window !== 'undefined' && window.QueryForgeConfig?.legacyFilterSunsetNote ? (
                <div style={{ marginTop: '12px', fontSize: '11px', opacity: 0.92, lineHeight: 1.45 }}>
                  {window.QueryForgeConfig.legacyFilterSunsetNote}
                </div>
              ) : null}
            </div>
          )}
                          ...
                          onClick={() => {
                            ...
                            try {
                              ...
                              if (graphData && graphData.nodes && graphData.edges) {
                                ...
                                setLegacyMigrationOverlayDismissed(false);
                                setNodes(withCpt);
