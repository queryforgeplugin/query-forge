...
function mountReactModal(widgetModel) {
  ...
  try {
    ...
    // Mount the App.
    let backupContextLabel = '';
    try {
      const doc =
        typeof elementor !== 'undefined' &&
        elementor.documents &&
        typeof elementor.documents.getCurrent === 'function'
          ? elementor.documents.getCurrent()
          : null;
      const t =
        doc?.config?.document_settings?.post_title ||
        (typeof doc?.container?.settings?.get === 'function'
          ? doc.container.settings.get('post_title')
          : '');
      if (t) {
        backupContextLabel = String(t);
      }
    } catch (e) {
      backupContextLabel = '';
    }
