...
import { normalizeFilterNodeData, isFilterNodeIncomplete } from './normalizeFilterNode.js';

/**
 * Build filter clause from FilterNode (normalized multi-condition or legacy).
 *
 * @param {Object} filterNode - Filter node.
 * @return {Object|null} Filter clause or grouped clause.
 */
function buildFilterClause(filterNode) {
  if (!filterNode || !filterNode.data) {
    return null;
  }
  const normalized = normalizeFilterNodeData(filterNode.data);
  ...
  const clauses = conditions
    .map((condition) => {
      ...
      if (String(op).toUpperCase() === 'BETWEEN' && condition.value && typeof condition.value === 'object' && !Array.isArray(condition.value)) {
        clause.value = {
          from: condition.value.from !== undefined ? condition.value.from : '',
          to: condition.value.to !== undefined ? condition.value.to : '',
        };
      } else if (condition.value !== undefined && condition.value !== '') {
        clause.value = condition.value;
      }
      if (condition.valueType) {
        clause.value_type = condition.valueType;
      }
      return clause;
    })
    .filter((c) => c.field);
...
export function transformToSchema(nodes, edges) {
  ...
  // Fix 2 — Connected filter nodes where every condition row is empty (no field and no value).
  nodes.forEach((node) => {
    if (node.type !== 'filter') return;
    if (unconnectedNodeIds.includes(node.id)) return;
    if (isFilterNodeIncomplete(node.data)) {
      unconnectedNodeIds.push(node.id);
    }
  });
