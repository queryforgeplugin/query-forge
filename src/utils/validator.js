...
import { isFilterNodeIncomplete } from './normalizeFilterNode.js';
...
export function validateGraph(nodes, edges) {
  ...
  filterNodes.forEach((fn) => {
    if (isFilterNodeIncomplete(fn.data)) {
      errors.push(`Filter node "${fn.data?.label || fn.id}" has no field or value set on any condition row. Add conditions or remove the node.`);
    }
  });
